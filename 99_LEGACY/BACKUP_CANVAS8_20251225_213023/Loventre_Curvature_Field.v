...
Module FT := Loventre_Foundation_Types.
Import Loventre_Geometry.

(** Alias coerenti con il motore Loventre. *)

Definition kappa_L (x : FT.State) : Z :=
  kappa x.

Definition alpha_c_L : Z :=
  alpha_c.

Definition curvature_subcritical (x : FT.State) : Prop :=
  subcritical x.

Definition curvature_supercritical (x : FT.State) : Prop :=
  supercritical x.
