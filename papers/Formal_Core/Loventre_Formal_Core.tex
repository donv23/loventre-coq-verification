...
This document presents the \emph{formal core} of the Loventre Model.
All results are internal to the model.
No claim is made regarding the classical P vs NP problem.
...
Let $\mathcal{V}$ be an abstract set of structural values equipped with a relation $\leq$.
\end{definition}
...
A trajectory $\tau \subseteq \mathcal{C}$ is said to be efficient if there exists $B \in \mathcal{V}$ such that for all invariants $f \in \{\kappa,H,\chi\}$ and all $c_1,c_2 \in \tau$,
\[
...
A computation $c \in \mathcal{C}$ exhibits a structural horizon if for every trajectory $\tau$ containing $c$ and reaching a target region $\mathcal{T}$, the trajectory is not efficient.
\end{definition}
...
\texttt{Loventre\_Formal\_Core.v}.
\end{document}
