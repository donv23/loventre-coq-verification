...
NEW_DEF = '''def append_policy_bridge_to_metrics(metrics: dict) -> dict:
    ...
    - Appende una riga di sintesi a meta_explanation, del tipo:
    ...
    try:
        ...
        warning = "Loventre Policy Bridge non disponibile (import fallito: {0})".format(e)
        metrics["policy_bridge_warning"] = warning
        return metrics

    # Parametri chiave dalle metrics.
    risk_index = metrics.get("risk_index")
    # Nota: K_globale spesso è definito a livello di famiglia; se manca qui usiamo default nel lab.
    ...
    try:
        ...
        warning = "Loventre Policy Bridge ha sollevato un'eccezione: {0}".format(e)
        metrics["policy_bridge_warning"] = warning
        return metrics

    # Estrarre i campi principali dalla decisione.
    ...
    if strategy is None and energy_policy is None and comment is None:
        warning = "Loventre Policy Bridge ha restituito un payload vuoto o non interpretabile."
        metrics["policy_bridge_warning"] = warning
        return metrics
    ...
    # Costruire la riga di sintesi per meta_explanation.
...
def patch_append_policy_bridge(path: Path) -> None:
    ...
    # Cerca la def esistente di append_policy_bridge_to_metrics
    start_idx = None
    ...
    # Trova l'inizio della prossima def a colonna zero (o EOF).
    end_idx = len(lines)
