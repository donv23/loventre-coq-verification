...
def append_hawking_layer_to_metrics(metrics: dict) -> dict:
    ...
    Prende metrics, chiama compute_hawking_layer e restituisce il dict aggiornato.
    Deve essere safe se i dati non sono disponibili.
    """
    try:
...
def ensure_hawking_hook_in_tail(code: str) -> str:
    ...
    if count == 0:
        print(
            "Attenzione: non ho trovato la riga con append_planck_layer_to_metrics(metrics) "
            "nel tail; nessun hook Hawking inserito."
        )
        return code
